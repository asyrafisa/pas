...
//       String dataLeaveStatus = dataGetLeaveStatus.get(0).get(2);
        ...
        String sqlValidateAppointment = "SELECT * "
                            ...
                            + "WHERE pmi_no = '"+pmiNo+"' AND status = 'active'";
...
//        out.print(sqlCheckApp);
        if(dataCheckApp.size()> 0) //if data exist in pms_appointment
        {
            %><script language='javascript'>
                alert('You cannot make a new appointment due to your last appointment request still active'); 
                    ...
                    if(dataGetRoster.size() > 0)
                    {
                        if(dataGetLeaveStatus.size() > 0)
                        {
                            %><script language='javascript'>
                                ...
                                window.location= 'patientAppointment.jsp?e27=<%=pmiNo%>&e31=<%=patientName%>&e30=<%=idNo%>&e32=<%=appDate%>&e33=<%=appTime%>&e34=<%=appDoc%>&e35=<%=disApp%>&e36=<%=subDisApp%>&e37=<%=appType%>';
                             </script> <%
                        }
                        else
                        {
                                ...
                                String sqlInsert = "INSERT INTO pms_appointment (pmi_no, hfc_cd, appointment_date, module_cd, hfc_to, txn_date, location_cd, userid, encounter_date, episode_date, order_no, appointment_type, register_status, status, remarks, start_time, end_time, discipline, subdiscipline) "
                                                + "VALUES ('"+pmiNo+"' , '"+dataHFCCode+"','"+appDate+"','-','-','0000-00-00','-','"+dataUserID+"','0000-00-00','0000-00-00','-','online','-','active','-','"+appDateTime+"','0000-00-00','"+disApp+"','"+subDisApp+"')";
